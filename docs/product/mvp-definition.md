...
- **目的**：绑定 Workspace，说明本地文件约定。
- **主要信息**：当前 Workspace 路径；约定扫描路径（如 `HANDOFF.md`、`AGENTS.md`、`.cursor/rules/`、skills 目录）；应用版本/平台信息（可选）。
- **主要操作**：选择文件夹；保存绑定；打开资源管理器定位到 Workspace（可选）。
- **暂不做**：主题商店、账号、云配置、全局加密密钥管理。
