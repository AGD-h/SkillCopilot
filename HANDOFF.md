...
- Follow-up pre-push fix: Cursor rules directories past `MAX_DEPTH=6` no longer disappear silently. When a subdirectory would require recursion beyond the depth limit, the scanner emits a warning that includes the skipped directory path and an explicit depth-limit reason; `warningCount` tracks the true total, retained warnings still cap at 100 (`warningsTruncated`), and `truncated` remains reserved for the 200 valid-unique Agent cap. Frontend `isPartial` already treats `warningCount > 0` as partial real data, so no new UI state was required. After the fix: `cargo fmt --check`, `cargo test` (70 passed: 38 Agent + 32 Skill), `cargo check`, `pnpm build`, and `git diff --check` all exited 0.
